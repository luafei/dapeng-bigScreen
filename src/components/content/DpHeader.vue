    ...
    <div class="titleLine" v-if="isShowMenu">
      ...
      <ul class="titleUl">
        <li  v-for="subTitle in dpHeaderList" :key="subTitle.id" :class="{active:activeOn=== subTitle.id}">
          ...
          <el-dropdown :hide-on-click="false"  v-if="subTitle.children.length>0">
            <span
              class="el-dropdown-link traffic-title"
            >
              {{subTitle.text}}
            ...
            <el-dropdown-menu slot="dropdown" class="traffic-pull-down"  :divided="false">
              <div v-for="route in subTitle.children" :key="route.path">
                <div v-if="route.meta && !route.hide">
                  <el-dropdown-item
                    class="traffic-pull-item"
                    :key="route.name"
                  >
                    <router-link  :to="route.path" exact>{{route.title}}</router-link>
  ...
  watch:{
    permission:{
      handler: function (vals){
        for (let index = 0; index < vals.routers.length; index++) {
          if(!vals.routers[index].children||vals.routers[index].children.length === 0) continue;
          const element = vals.routers[index];
          for (let i = 0; i < this.dpHeaderList.length; i++) {
            if(element.name === this.dpHeaderList[i].text){
              this.dpHeaderList[i].children = element.children;
            }
          }
        }
      },
